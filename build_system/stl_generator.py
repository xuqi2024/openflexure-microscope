...
def generate_platform_optics_modules(writer):
    ...
    for camera, optics in PLATFORM_OPTICS_MODULE_OPTIONS:
        ...
        writer.openscad(output, "lens_spacer.scad", parameters, select_stl_if=select_stl_if)
...
def generate_no_pi_stand(writer):
    ...
    writer.openscad("microscope_stand_no_pi.stl",
...
def generate_gears_and_thumwheels(writer):
    ...
    writer.openscad("small_gears.stl", "small_gears.scad", select_stl_if=small_gear_selected)
    writer.openscad("gears.stl", "gears.scad",select_stl_if=large_gear_selected)
    writer.openscad("thumbwheels.stl", "thumbwheels.scad", select_stl_if=thumbwheels_selected)
...
def generate_small_parts(writer):
    ...
    writer.openscad("slide_riser.stl", "slide_riser.scad", select_stl_if={"slide_riser": True})
    ...
    writer.openscad("fl_cube.stl", "fl_cube.scad", select_stl_if={"reflection_illumination": True})
    ...
    writer.openscad("actuator_drilling_jig.stl",
                    "actuator_drilling_jig.scad",
                    select_stl_if={"include_actuator_drilling_jig": True})
    writer.openscad("reflection_illuminator.stl",
...
def add_stls_to_writer(writer):
    ...
    generate_bases(writer)
    generate_gears_and_thumwheels(writer)
    generate_small_parts(writer)
