    ...
    difference(){
        union(){
            ...
            sequential_hull(){
                ...
                union(){
                    translate([-3/2, l-12,0]){
                        cube([3,12,h]);
                    }
                    translate([-7/2, l-12,h-1]){
                        cube([7,12,1]);
                    }
                }
            }
...
module blade_point(pos, d1=1.5, d2=1.5, h=tiny()){
    union(){
        translate(blade_anchor + [0,0,pos.z]){
            cylinder(d=d1, h=h);
        }
        translate(pos){
            cylinder(d=d2, h=h);
            ...
            hull(){
                reflect([1,0,0]){
                    prong_frame(){
                        ...
                        translate(blade_anchor + [-1.5,10,0.5]){
                            cube([1.5/2,999,999]);
                        }
                    }
                }
...
module double_ended_band_tool(bent=false){
    ...
    middle_w = 2*column_base_radius()+1.5+2*(band_tool_h-roc)+0.5;
