...
function picamera_3_camera_dict() = [["mount_height", 7], // height of the basic camera mount, camera height from the board (5mm) plus 2mm
                                     ["sensor_height", 2]];//Height of the sensor above the PCB

function picamera_3_bottom_z() = -key_lookup("mount_height", picamera_3_camera_dict());

function picamera_3_hole_spacing() = 21;

//picam3_flex_and_components(camera_width=11+1);

// module picam3_flex_and_components(camera_width=11.5+1){
//     // A 2D perimeter inside which the flex and components of the camera sit.
//     // 
//     // camera_width is the width of the camera module's casing, nominally 11 or 11.5mm but
...
module picam3_flex_and_components(){
    // A 2D perimeter inside which the flex and components of the camera sit.
    // 
...
//         cube(30, center=true);
//     }
//     picam3_cutout( beam_length=15);
// }

module picam3_cutout( beam_length=15){
    ...
    mount_height = key_lookup("mount_height", picamera_3_camera_dict());
    ...
    union(){
        ...
        // transition from teh square camera mount to a cylindrical optical cut-out
        translate_z(camera_height){
            hole_from_bottom(r=hole_r, h=2*mount_height, base_w=camera_width, delta_z=0.5, layers=4, big_bottom=false);
        }
        ...
        //hull(){
            translate_z(-tiny()){
                linear_extrude(flex_h){
                    // picam3_flex_and_components(9.5+0*camera_width);
                    picam3_flex_and_components();
                }
            }
            // #translate_z(-tiny()){
            //     linear_extrude(flex_h+extra_h){
            //         offset(-extra_h){
            //             picam3_flex_and_components(camera_width);
            //         }
            //     }
            // }
            //cylinder(r=hole_r, h=2*mount_height, center=true);
...
module picamera_3_camera_mount(screwhole=true, counterbore=false){
    // A mount for the pi camera module 3
    ...
    difference(){
        rotate(45){
            translate_y(2.4){
                sequential_hull(){
                    translate_z(picamera_3_bottom_z()){
                        picam2_board(h=tiny());
                    }
                    translate_z(-1){
                        picam2_board(h=tiny());
                    }
                    cube([w-(-1.5-picamera_3_bottom_z())*2,b,tiny()],center=true);
                }
            }
        }
        rotate(45){
        ...
        if(screwhole){
            picamera_3_screwholes();
        }
    }
}

module picamera_3_screwholes(){
    //chamfered screw holes for mounting
    screw_x = picamera_3_hole_spacing()/2;
    rotate_z(45){
        translate_z(picamera_3_bottom_z()){
            reflect_x(){
                translate_x(screw_x){
...
module picamera_3_counterbore(){
    translate_z(picamera_3_bottom_z()-1){
        picamera_3_bottom_mounting_posts(height=999, radius=1.25, cutouts=false);
    }
    translate_z(picamera_3_bottom_z()+1){
        picamera_3_bottom_mounting_posts(height=999, radius=2.8, cutouts=false);
    }
}

module picamera_3_bottom_mounting_posts(height=-1, radius=-1, outers=true, cutouts=true){
