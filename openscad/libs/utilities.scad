...
module lighttrap_cylinder(r1, r2, h, ridge=1.5){
    ...
    // The previous function used the number of points in `cylinder()`.  This follows
    // logic set out in:
    // https://en.wikibooks.org/wiki/OpenSCAD_User_Manual/Other_Language_Features#Special_variables
    // This gives a following fragment of code to calculate the number of points.
    // I've duplicated it below, split over a couple of lines to aid readability.
    default_n_points = ceil(max(min(360/$fa,r1*2*PI/$fs),5)); // use minimum size or maximum angle
    n_points = max($fn>0?$fn:default_n_points, 3); // $fn takes precedence, with minimum of 3
    function ring_points(r, z) = [
        for(i=[1:n_points]) [r*cos(360*i/n_points), r*sin(360*i/n_points), z]
    ];
    function wrap(j) = j % n_points; // wrap when indexing around the circle
    assert(wrap(n_points)==0, "Error in wrap() internal function");
    r1_g = (r2 + ridge - r1)/(n_cones - 1);
    r2_g = (r1 - ridge - r2)/(n_cones - 1);
    function outer_ring_i(ring_number) = n_points*ring_number;
    function inner_ring_i(ring_number) = n_points*(n_cones + ring_number);
    bottom_i = 2*n_cones*n_points;
    top_i = bottom_i+1;
    polyhedron(
        points=[
            // All the points on the outer edges, i.e. r1 at the bottom 
            // For each cone, these are the wider, bottom radii
            for(i=[0:n_cones-1]) let(
                p=i/(n_cones-1),        // p=0 on bottom cone, 1 on top cone
                q=1-p,                  // p=1 on bottom cone, 0 on top cone
                r=r1*q + (r2+ridge)*p,  // radius of each ring - constant if r1=r2+ridge
                z=i*cone_h - tiny()     // NB these are the *bottoms* of the cones
            ) each ring_points(r, z),

            // All the points on the inner edges, i.e. r2 at the top 
            // For each cone, these are the narrower, top radii
            for(i=[0:n_cones-1]) let(
                p=i/(n_cones-1),        // p=0 on bottom cone, 1 on top cone
                q=1-p,                  // p=1 on bottom cone, 0 on top cone
                r=(r1-ridge)*q + r2*p,  // radius of each ring - constant if r1=r2+ridge
                z=(i+1)*cone_h - tiny() // NB these are the *tops* of the cones
            ) each ring_points(r, z),

            // Extra points at the top and bottom, 
            [0,0,-tiny()],  //bottom
            [0,0,h+tiny()]  //top
        ],
        faces=[
            // Sloping faces
            for(i=[0:n_cones - 1]) for(j=[0:n_points - 1]) [
                outer_ring_i(i) + j,
                inner_ring_i(i) + j,
                inner_ring_i(i) + wrap(j + 1),
                outer_ring_i(i) + wrap(j + 1)
            ],
            // Flat faces - NB there are n_cones-1 rings of these
            for(i=[0:n_cones - 2]) for(j=[0:n_points - 1]) [
                inner_ring_i(i) + j,
                outer_ring_i(i+1) + j,
                outer_ring_i(i+1) + wrap(j + 1),
                inner_ring_i(i) + wrap(j + 1)
            ],
            // Bottom face, made up of n_points triangles
            for(j=[0:n_points - 1]) [
                outer_ring_i(0) + j,
                outer_ring_i(0) + wrap(j + 1),
                bottom_i
            ],
            // Top face, made up of n_points triangles
            for(j=[0:n_points-1]) [
                inner_ring_i(n_cones - 1) + wrap(j + 1),
                inner_ring_i(n_cones - 1) + j,
                top_i
            ]
        ]
    );
