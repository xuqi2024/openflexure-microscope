...
function rms_f50d13_galvo_config(camera_type = "picamera_2", beamsplitter=false, parfocal_distance=45) = let(
    // if the parfocal distance is undefined, use the default.
    parfocal_distance_validated = (parfocal_distance == undef) ? 45 : parfocal_distance,


    lens_objective_distance = 100,          //make this 2f for a 50mm lens so that we have a 2f system with collimated light entering from above. TODO don't leave this hardcoded!
    config_dict = [["optics_type", "RMS"],
                   ["camera_type", camera_type],
                   ["tube_lens_ffd", 47],
                   ["tube_lens_f", 50],
